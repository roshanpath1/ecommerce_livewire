        ...
        <div class="flex flex-col gap-4 md:flex-row">
                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key='{{ $item['product_id'] }}'>
                                    ...
                                    {{-- @if ($item->product->quantity>$item->product->qty) --}}
            ...
            <div class="md:w-1/4">
                <div class="p-6 bg-white rounded-lg shadow-md">
                    ...
                    <div class="flex justify-between mb-2">
                        <span>Taxes</span>
                        <span>{{ Number::currency($taxes, 'NPR') }}</span>
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Shipping</span>
                        <span>{{ Number::currency($shipping, 'NPR') }}</span>
                    </div>
                    <hr class="my-2">
                    <div class="flex justify-between mb-2">
                        <span class="font-semibold">Grand Total</span>
                        <span class="font-semibold">{{ Number::currency($grand_total+ $shipping+$taxes, 'NPR') }}</span>
                    ...
                    @if ($cart_items)
                    <a href="/checkout"
                        class="block w-full px-4 py-2 mt-4 text-center text-white bg-blue-500 rounded-lg">Checkout</a>
                        @endif
