            ...
            <div class="flex items-center mt-1 gap-x-2">
                @php
                    ...
                    if($order->order->status=='shipped'){
                         $status=' <span class="px-3 py-1 text-white bg-green-700 rounded shadow">Shipped</span> ';
                    }
                ...
                @php
                  $payment_status='';
                    ...
                    if($order->order->payment_status=='paid'){
                        $payment_status='<span class="px-3 py-1 text-white bg-green-700 rounded shadow">Paid</span>';
                    }
      ...
      <div class="md:w-3/4">
                ...
                <tr wire:key="{{$order->id}}">
                    <td class="py-4">
                      <div class="flex items-center">
                        <img class="w-16 h-16 mr-4" src="{{url('storage',$order->product->image)}}" alt="{{$order->product->name}}">
                        <span class="font-semibold">{{$order->product->name}}</span>
                      </div>
                    </td>
                    <td class="py-4">{{Number::currency($order->unit_amount,'NPR')}}</td>
                    <td class="py-4">
                      <span class="w-8 text-center">{{$order->quantity}}</span>
                    </td>
                    <td class="py-4">{{Number::currency($order->total_amount,'NPR')}}</td>
                  </tr>
        ...
        <div class="p-6 mb-4 overflow-x-auto bg-white rounded-lg shadow-md">
          ...
          <div class="flex items-center justify-between">
            <div>
              <p>{{$address->street_address}},{{$address->city}},{{$address->country}}</p>
            </div>
            <div>
      ...
      {{-- <div class="md:w-1/4">
        <div class="p-6 bg-white rounded-lg shadow-md">
          ...
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>{{Number::currency($order->grand_total,'NPR')}}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxes</span>
